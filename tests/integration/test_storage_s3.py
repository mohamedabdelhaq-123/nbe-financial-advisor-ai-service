...
async def test_write_read_exists_list_delete_round_trip(real_s3_storage_env):
    ...
    async with get_storage_backend() as s3:
        await s3.put_object(Bucket=settings.storage_s3_bucket, Key=key, Body=payload)

        await s3.head_object(Bucket=settings.storage_s3_bucket, Key=key)

        response = await s3.get_object(Bucket=settings.storage_s3_bucket, Key=key)
        async with response["Body"] as stream:
            body = await stream.read()
        assert body == payload

        listing = await s3.list_objects_v2(Bucket=settings.storage_s3_bucket, Prefix=prefix)
        keys = [obj["Key"] for obj in listing.get("Contents", [])]
        assert keys == [key]

        await s3.delete_object(Bucket=settings.storage_s3_bucket, Key=key)
