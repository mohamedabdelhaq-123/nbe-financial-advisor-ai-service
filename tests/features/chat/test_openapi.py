...
def test_chat_response_documents_envelope_and_examples(openapi: dict):
    ...
    assert {"token", "done", "error", "tool_call"} <= set(envelope["properties"]["event"]["enum"])

    # All four event frames appear as examples.
    examples = sse["examples"]
    assert {"token", "done", "error", "tool_call"} <= set(examples)
    for name in ("token", "done", "error", "tool_call"):
