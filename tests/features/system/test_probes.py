...
def test_mineru_health_mock_mode(client: TestClient, monkeypatch):
    ...
    monkeypatch.setattr(settings.mineru, "use_mock", True)
...
def test_mineru_health_real_unreachable(client: TestClient, monkeypatch):
    ...
    monkeypatch.setattr(settings.mineru, "use_mock", False)
    monkeypatch.setattr(settings.mineru, "api_url", "http://127.0.0.1:1")
    response = client.get("/health/mineru")
    assert response.status_code == 200
    body = response.json()
