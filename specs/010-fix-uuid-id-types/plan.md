...
| **VI. LLM & Agent Architecture** | No change to the Maestro/sub-agent architecture or the set of nodes/edges in the LangGraph pipeline. D11 does alter agent-internal behavior, however: `is_first_turn` is removed and `chat/service.py` now calls `graph.aget_state(config)` unconditionally on every turn, restoring `planner_answers`, `questions_asked`, `stage`, and `user_context` from checkpointer state rather than gating that read on a client-supplied flag — changing chat-state restoration semantics. The type cleanup (drop a cast, swap a fabricated name for a fetched one) is unchanged. Coverage: `tests/integration/test_chat_memory.py` and `tests/features/chat/test_analysis_agent.py` exercise multi-turn continuity and agent state without `is_first_turn`. | PASS — sub-agent architecture unchanged; the D11 state-restoration change is covered by tests and stays within agent-internal behavior (no new node/edge/checkpointer store). |
